...
3. raw数据中缺失的字段不要编造，在解读中标注数据缺失即可
4. 分析部分标注⚠️分析判断
5. 只输出JSON，不要其他文字

## 自检报告规范
- unverified_list只列「每日应该获取但今天缺失」的数据项（如北向资金、两融余额、外汇汇率、商品价格）
- 月度数据（CPI/PMI/M2等）如果当日没有新发布，不算缺失，不需列入unverified_list
- news数据中获取到的数据标注来源，不算未验证
- 预计总数控制在5-8项以内，不要列十几项`;
